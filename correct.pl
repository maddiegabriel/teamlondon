...
while ($programFlag == 0) {
    while ($mainFlag == 0) {
        ...
        #  THIRD QUESTION
           ...
           if($high_low == 1) {
                if($geo == 1) {
                   print("\nThe top $n provinces with the highest rates of $violation_name from $start year to $end_year were:\n");
                   for($i = 0; $i < $n; $i++) {
                       @split_it = split (/\./, @$coords[$i]);
                       my $location = location_name($split_it[0]);
                    ...
                    print("\nThe top $n cities with the highest rates of $violation_name from $start year to $end_year were:\n");
                   for($i = 1; $i <= $n; $i++) {
                       @split_it = split (/\./, @$coords[$i]);
                       my $location = location_name($split_it[0]);
                       print("\n",$i+1,".   $location: @$percents[$i]%.");
                ...
                if($geo == 1) {
                   print("\nThe bottom $n provinces with the lowest rates of $violation_name from $start year to $end_year were:\n");
                   for($i = ($size - $n + 1); $i <= $size; $i++) {
                       @split_it = split (/\./, @$coords[$i]);
                       my $location = location_name($split_it[0]);
                       print("\n",$i+1,".   $location: @$percents[$i]%.");
                   ...
                   print("\nThe bottom $n cities with the lowest rates of $violation_name from $start year to $end_year were\n");
                   for($i = ($size - $n + 1); $i <= $size; $i++) {
                       @split_it = split (/\./, @$coords[$i]);
                       my $location = location_name($split_it[0]);
                       print("\n",$i+1,".   $location: @$percents[$i]%.");
