...
sub location_name {
    ...
    if ($coordinate_location == 41) {
        ...
        $location = "Abbotsford-Mission, BC";
...
while ($programFlag == 0) {
    while ($mainFlag == 0) {
        print "\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  QUESTION TYPES  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"; 
        print "\nQuestion Set 1: What province/city had the highest/lowest rate per 100,000 population of a violation in a year?\n\n";
        print "Question Set 2: What province/city had the highest/lowest percent change in the rate a violation between two years?\n\n";
        print "Question Set 3: What are the top n provinces/cities with the highest/lowest rate per 100,000 population of a violation in a year?\n\n";
        print "Question Set 4: What are the top n provinces/cities with the highest/lowest percent change in the rate a violation between two years?\n\n";
        ...
        if ($setQ == 1) {
            ...
            while ($flag == 0){
                ...
                $violation = <STDIN>;
                chomp $violation;
                if ($violation == 4 || $violation == 6 || $violation == 8 || $violation == 148 || $violation == 16 || $violation == 17 || $violation == 29 || $violation == 30 || $violation == 34 || $violation == 38 || $violation == 79 || $violation == 84 || $violation == 99 || $violation == 149|| $violation == 231 || $violation == 120) {
                    $flag = 1;
                }
            }
            ...
            if($high_low == 1) {
                ...
                if($geo == 1) {
                   ...
                   print("\nANSWER: The province with the highest rate of $violation_name in $year was $location. The rate was @$values[0] per 100,000 population.\n\n");                   
                } elsif ($geo == 2) {
                   print "\n\nYOUR QUESTION: What city had the highest Rate per 100,000 Population of $violation_name in $year?\n";
                   print("\nANSWER: The city with the highest rate of $violation_name in $year was $location. The rate was @$values[0] per 100,000 population.\n\n");                   
                }
            } else {
                ...
                if ($geo == 1) {
                   ...
                   print("\nANSWER: The province with the lowest rate of $violation_name in $year was $location. The rate was @$values[$size] per 100,000 population.\n\n");                   
                } elsif ($geo == 2) {
                   print "\n\nYOUR QUESTION: What city had the lowest Rate per 100,000 Population of $violation_name in $year?\n";
                   print("\nANSWER: The city with the lowest rate of $violation_name in $year was $location. The rate was @$values[$size] per 100,000 population.\n\n");                   
                }
            }
           print "\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ NEXT QUESTION ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n\n"; 
        #   SECOND QUESTION
               ...
               while ($flag == 0){    
                   ...
                   $violation = <STDIN>;
                   chomp $violation;
                   if ($violation == 4 || $violation == 6  || $violation == 148|| $violation == 8 || $violation == 16 || $violation == 17 || $violation == 29 || $violation == 30 || $violation == 34 || $violation == 38 || $violation == 79 || $violation == 84 || $violation == 99 || $violation == 149|| $violation == 231 || $violation == 120) {
                       $flag = 1;
                   }
               }
               ...
               while ($flag == 0) {
                   print "Start year (1998 - 2014): ";
                   $start_year = <STDIN>;
                   chomp $start_year;
                   if ($start_year >= 1998 && $start_year <= 2014) {
                       $flag = 1;
                   }
               }
               $flag = 0;
               ...
               $flag = 0;
                ...
                if($high_low == 1) {
                    ...
                    $location = location_name($split_it[0]);
                    if($geo == 1) {
                       print "\n\nYOUR QUESTION: What province had the highest percent change of $violation_name from $start_year to $end_year?\n";
                       print("\nANSWER: The province with the highest percent change of $violation_name between $start_year and $end_year was $location. The percent change was @$percents[0]%.\n");                   
                    } elsif ($geo == 2) {
                       print "\n\nYOUR QUESTION: What city had the highest percent change of $violation_name from $start_year to $end_year?\n";
                       print("\nANSWER: The city with the highest percent change of $violation_name between $start_year and $end_year was $location. The percent change was @$percents[0]%.\n");                   
                    }
                } else {
                    ...
                    $location = location_name($split_it[0]);
                    if ($geo == 1) {
                        print "\n\nYOUR QUESTION: What province had the lowest percent change of $violation_name from $start_year to $end_year?\n";
                       print("\nANSWER: The province with the lowest percent change of $violation_name between $start_year and $end_year was $location. The percent change was @$percents[$size]%.\n");                   
                    } elsif ($geo == 2) {
                        print "\n\nYOUR QUESTION: What city had the lowest percent change of $violation_name from $start_year to $end_year?\n";
                       print("\nANSWER: The city with the lowest percent change of $violation_name between $start_year and $end_year was $location. The percent change was @$percents[$size]%.\n");                   
                    }
                }
               print "\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ NEXT QUESTION ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n"; 
        #  THIRD QUESTION
           ...
           if($high_low == 1) {
                if($geo == 1) {
                   ...
                   for($i = 0; $i < $n; $i++) {
                       ...
                       my $location = sprintf("%-25s", location_name($split_it[0]));
                       my $num = sprintf("%02d", $i+1);
                       my $valuez = sprintf("%.3f", @$values[$i]);
                       print("\n$num.   $location $valuez per 100,000 population.");
                   } 
                } elsif ($geo == 2) {
                    print("\nThe top $n cities with the highest rates of $violation_name in $year are:\n");
                   for($i = 1; $i <= $n; $i++) {
                       @split_it = split (/\./, @$coords[$i]);
                       my $location = sprintf("%-33s", location_name($split_it[0]));
                       my $num = sprintf("%02d", $i+1);
                       my $valuez = sprintf("%.3f", @$values[$i]);
                       print("\n$num.   $location $valuez per 100,000 population.");
                   } 
           ...
           while ($flag == 0){
               ...
               $violation = <STDIN>;
               chomp $violation;
               if ($violation == 4 || $violation == 148 || $violation == 6 || $violation == 8 || $violation == 16 || $violation == 17 || $violation == 29 || $violation == 30 || $violation == 34 || $violation == 38 || $violation == 79 || $violation == 84 || $violation == 99 || $violation == 149|| $violation == 231 || $violation == 120) {
                   $flag = 1;
               }
           }
           ...
           if($high_low == 1) {
                if($geo == 1) {
                   ...
                   for($i = 0; $i < $n; $i++) {
                       ...
                       my $location = sprintf("%-25s", location_name($split_it[0]));
                       my $num = sprintf("%02d", $i+1);
                       my $percentz = sprintf("%.3f", @$percents[$i]);
                       print("\n$num.   $location $percentz%.");
                   } 
                } elsif ($geo == 2) {
           ...
           print "\nFILL IN THE BLANKS: Construct a line graph for the percent change of a violation in a specific location between 2 years.\n";
           while ($flag == 0) {
               print "\nProvince or city? (1 for province, 2 for city): ";
               $general_geo = <STDIN>;
               chomp $general_geo;
            ...
            while ($flag == 0) {
               ...
               if ( $geo != '') {
                    if( $general_geo == 1 && ($geo == 33 || $geo == 36 || $geo == 28 || $geo == 7 || $geo == 2 || $geo == 5 || $geo == 41 || $geo == 42 || $geo == 16 || $geo == 4 || $geo == 9 || $geo == 30 || $geo == 40)) {
                        $flag = 1;
                    } elsif ($general_geo == 2 && ($geo == 3 || $geo == 6 || $geo == 43 || $geo == 11 || $geo == 14 || $geo == 19 || $geo == 29 || $geo == 31 || $geo == 34 || $geo == 35 || $geo == 37 || $geo == 38)) {
                        $flag = 1;
                    } 
               }
           }
